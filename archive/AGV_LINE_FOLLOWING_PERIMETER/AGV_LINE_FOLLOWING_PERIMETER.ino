...
// Right turn behavior.
// This uses the rotateTo-style yaw controller, not alvik.rotate().
...
// Small non-blocking brake pauses around the turn
...
// =====================================================
//
// Coordinate convention used by your test script:
//
// The AGV starts at row 1, col 1.
// The AGV starts facing +Y.
// The first movement increases COLUMN.
//
// Route:
//
// Start: row 1, col 1
...
// Non-blocking red LED flash
bool red_flash_active = false;
unsigned long red_flash_start_ms = 0;

// Non-blocking final blink
enum ResultBlinkPhase {
  RESULT_BLINK_WAIT,
  RESULT_BLINK_ON,
  RESULT_BLINK_OFF,
  RESULT_BLINK_COMPLETE
};

bool blink_initialized = false;
unsigned long blink_phase_start_ms = 0;
int blink_index = 0;
ResultBlinkPhase result_blink_phase = RESULT_BLINK_WAIT;
...
void driveRouteState() {
  ...
  if (countRedIfNeeded(red_now)) {
    ...
    if (shouldTurnRightHere()) {
      Serial.println("Corner reached. Preparing non-blocking rotateTo-style right turn.");
...
void turnRightState() {
  ...
  switch (turn_phase) {
    ...
    case TURN_CENTER_FORWARD:
      ...
      if (now - turn_phase_start_ms >= TURN_CENTERING_MS) {
        alvik.brake();
    ...
    case TURN_PRE_ROTATE_BRAKE:
      ...
      if (now - turn_phase_start_ms >= PRE_ROTATE_BRAKE_MS) {
        Serial.println("Turning right with rotateTo-style yaw control...");
        startRotateRelative(RIGHT_TURN_DEG);
    ...
    case TURN_ROTATING:
      ...
      if (updateRotateTo()) {
        ...
        Serial.print("Turn complete | Commanded: ");
        Serial.print(RIGHT_TURN_DEG);
        Serial.print(" deg | Time: ");
        Serial.print(elapsed_ms);
        Serial.print(" ms | Start yaw: ");
        Serial.print(turn_start_yaw, 2);
        Serial.print(" deg | End yaw: ");
        Serial.print(yaw, 2);
        Serial.print(" deg | Change: ");
        Serial.print(yaw - turn_start_yaw, 2);
        Serial.println(" deg");
    ...
    case TURN_POST_EXIT:
      ...
      if (now - turn_phase_start_ms >= POST_TURN_EXIT_MS) {
        alvik.brake();
...
bool updateRotateTo() {
  ...
  // Keep MAX_SPEED as the real maximum.
  // Without this cap, a 180-degree error can command higher than TURN_MAX_SPEED.
  if (scale > 1.0) {
    scale = 1.0;
  }
...
void startRedFlash() {
  red_flash_active = true;
  red_flash_start_ms = millis();
}
...
void updateRedFlash() {
  ...
  unsigned long elapsed = millis() - red_flash_start_ms;
  ...
  if (elapsed < 80) {
    ...
    red_flash_active = false;
  }
}

// =====================================================
// NON-BLOCKING FINAL RESULT BLINK
// =====================================================

void blinkResultState() {
  alvik.brake();
  ...
  if (!blink_initialized) {
    ...
    blink_done = false;
    blink_index = 0;
    result_blink_phase = RESULT_BLINK_WAIT;
    blink_phase_start_ms = millis();
  ...
  switch (result_blink_phase) {

    case RESULT_BLINK_WAIT:
      if (now - blink_phase_start_ms >= 500) {
        if (red_count <= 0) {
          setLEDGreen();
          blink_done = true;
          result_blink_phase = RESULT_BLINK_COMPLETE;
          robot_state = DONE;
        } else {
          setLEDRed();
          result_blink_phase = RESULT_BLINK_ON;
          blink_phase_start_ms = now;
        }
      }
      break;

    case RESULT_BLINK_ON:
      if (now - blink_phase_start_ms >= 350) {
        setLEDOff();
        result_blink_phase = RESULT_BLINK_OFF;
        blink_phase_start_ms = now;
      }
      break;

    case RESULT_BLINK_OFF:
      if (now - blink_phase_start_ms >= 350) {
        blink_index++;

        if (blink_index >= red_count) {
          setLEDGreen();
          blink_done = true;
          result_blink_phase = RESULT_BLINK_COMPLETE;
          robot_state = DONE;
