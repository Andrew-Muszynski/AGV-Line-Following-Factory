...
unsigned long last_follow_ms = 0;
...
void followLineOrDriveStraight(int l, int c, int r, float base_speed) {
  ...
  float dt = (last_follow_ms == 0) ? 0.001f
             : constrain((now_ms - last_follow_ms) / 1000.0f, 0.001f, 0.05f);
  last_follow_ms = now_ms;

  float error  = calculateCenterError(l, c, r);
  float d_term = (error - last_line_error) / dt;
  last_line_error = error;

  float yaw_err   = yawError(leg_target_yaw, yaw);
  float yaw_blend = KP_YAW_BLEND * yaw_err;

  float correction = constrain(error * KP + d_term * KD + yaw_blend,
